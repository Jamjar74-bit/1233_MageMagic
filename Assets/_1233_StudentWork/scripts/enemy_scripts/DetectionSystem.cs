...
public class DetectionSystem : MonoBehaviour
{
    ...
    [SerializeField] private LayerMask __obstructionMask;
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        var righttDir = Quaternion.Euler(0, -_fieldOfView / 2, 0) * transform.forward;
    ...
    public bool HasLineOfSight(Transform target, Vector3 directionToTarget)
    {
        if (target == null) return false;

        var directinToTarget = (target.position - _eyePosition.position).normalized;
        ...
        if (Physics.Raycast(_eyePosition.position, directionToTarget,
                out var hit, distanceToTarget, __obstructionMask))
    ...
    internal bool HasLineOfSight(Transform target)
    {
