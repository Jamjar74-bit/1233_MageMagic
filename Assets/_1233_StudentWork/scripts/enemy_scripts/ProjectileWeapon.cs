...
{
    [SerializeField] private Projectile _projectilePrefab;  // Changed in 1 asset
    [SerializeField] private Transform _muzzle;    // Muzzle (Transform)
    [SerializeField] private float _fireRate = 1f;    // Unchanged
    [SerializeField] private float _arcHeight = 2f;    // Unchanged

    private float _nextFireTime;

    public bool CanFire => Time.time >= _nextFireTime;

    #region Frequently called
